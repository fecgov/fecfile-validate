...
def __main__():
		...
		if AskTrueFalse("Would you like to delete the .angular and node_modules directories within "+FRONT_END_ROOT_DIR):
				Delete_App_Cache()

		docker_images = ["fecfile-db","fecfile-api","fecfile-celery-worker", "redis:6.2-alpine"]
		if AskTrueFalse("Would you like to delete the following docker images?\n"+"\n".join(docker_images)+"\n"):
