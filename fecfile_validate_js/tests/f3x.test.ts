...
Deno.test({
  ...
  fn: async () => {
    const result = await validate(f3xSchema, perfectForm_F3X);
    assertEquals(result, []);
  },
});
...
Deno.test({
  ...
  fn: async () => {
    ...
    const result = await validate(f3xSchema, thisData);
...
Deno.test({
  ...
  fn: async () => {
    ...
    delete thisData.date_signed;
    const result = await validate(f3xSchema, perfectForm_F3X, fieldsToValidate);
    assertEquals(result, []);
  },
});
...
Deno.test({
  ...
  fn: async () => {
    ...
    const fieldsToValidate = ["form_type"];
    delete thisData.form_type;
    const result = await validate(f3xSchema, thisData, fieldsToValidate);
    assertEquals(result[0].keyword, "required");
    assertEquals(result[0].path, "form_type");
    assertEquals(result[0].params.missingProperty, "form_type");
  },
